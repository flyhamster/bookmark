...
function getBookmarks(query) {
  $.getJSON(chrome.runtime.getURL('bookmarks.json'), function(data) {
    $('#bookmarks').append(getTreeNodes(data.bookmarks));
  });
}

function getTreeNodes(bookmarks, query) {
  var list = $('<ul>');
  for (var i = 0; i < bookmarks.length; i++) {
    list.append(getNode(bookmarks[i], query));
  }
  return list;
}
